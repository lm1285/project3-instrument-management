...
// Windows' legacy MAX_PATH limit rejects otherwise valid task paths. The
// extended-length prefix lets Node access existing long paths while remaining
// a no-op on Linux/macOS.
export const toFilesystemPath = (filePath: string) => {
  const value = String(filePath || '');
  if (process.platform !== 'win32' || !path.isAbsolute(value) || value.startsWith('\\\\?\\')) return value;
  if (value.startsWith('\\\\')) return `\\\\?\\UNC\\${value.slice(2)}`;
  return `\\\\?\\${value}`;
};
const removePath = (filePath: string, options: fs.RmOptions) => fs.promises.rm(toFilesystemPath(filePath), options);
const existsPath = (filePath: string) => fs.existsSync(toFilesystemPath(filePath));
const taskFolderPath = (taskId: string) => path.join(TASK_DIR, taskId);
...
export const normalizeMatchKeyword = (value: unknown) => String(value ?? '')
  ...
  .replace(/[()]/g, '')
  .replace(/(?:不带标|带标)$/u, '');
const splitMatchValueParts = (value: unknown) => String(value ?? '')
  .normalize('NFKC')
  .split(/[+,，、;；/\\|｜:：()\[\]【】]+/u)
  .map((part) => normalizeMatchKeyword(part))
  .filter(Boolean);
export const matchesTargetKeyword = (configured: unknown, actual: unknown) => {
  ...
  const actualParts = splitMatchValueParts(actual);
  return splitMatchKeywords(configured)
    .map(normalizeMatchKeyword)
    .filter(Boolean)
    .some((keyword) => normalizedActual === keyword || actualParts.includes(keyword));
};
...
const normalizeForcedKey = (value: unknown) => {
  ...
  if (key === '检定日期' || key === '校准时间') return '校准日期';
  if (key === '证书名称') return '证书单位';
  return key;
};
const SHARED_FORCED_KEYS = new Set(['证书单位', '证书地址', '校准日期', '特殊要求']);
...
function readRows(filePath: string, headerRow: number, dataStartRow: number) {
  const workbook = XLSX.readFile(toFilesystemPath(filePath), { cellDates: false });
...
function readHeader(filePath: string, headerRow: number, dataStartRow: number): string[] {
  try {
    const stat = fs.statSync(toFilesystemPath(filePath));
...
export async function writeTemplateText(templatePath: string, outputPath: string, updates: Map<string, unknown>) {
  const zip = await JSZip.loadAsync(await fs.promises.readFile(toFilesystemPath(templatePath)));
  ...
  await fs.promises.writeFile(toFilesystemPath(outputPath), await zip.generateAsync({ type: 'nodebuffer', compression: 'DEFLATE' }));
}
...
class OneClickTransferService {
  ...
  async detectSourceFile(file: Express.Multer.File | Buffer) {
    ...
    const toCandidate = (type: SourceDetectionCandidate['type'], groupName: string, item: any): SourceDetectionCandidate | null => {
      if (!item.file_path || !existsPath(item.file_path) || (type === 'order' && isLegacyBusinessType(item.template_item_name || item.type_name))) return null;
  ...
  /**
   * Target-template forced fields are shared by every source template.
   * Source-specific rows may override ordinary field mappings, but must not
   * hide the target item's shared certificate/unit/date/requirements slots.
   */
  private async getTargetMappings(targetTemplateId: string, sourceTemplateId: string) {
    ...
    const global = await this.db.all<any[]>(
      'SELECT * FROM transfer_mappings WHERE target_template_id = ? AND upload_template_id IS NULL ORDER BY rowid',
      [targetTemplateId],
    );
    if (!specific.length) return global;
    const sharedForced = global.filter((item) => SHARED_FORCED_KEYS.has(normalizeForcedKey(item.forced_key)));
    const specificForcedKeys = new Set(specific.filter((item) => item.forced_key).map((item) => normalizeForcedKey(item.forced_key)));
    return [
      ...specific,
      ...sharedForced.filter((item) => !specificForcedKeys.has(normalizeForcedKey(item.forced_key))),
    ];
  }
  ...
  private async rebuildPreviewRows(file: any): Promise<Record<string, unknown>[]> {
    try {
      if (!file.file_path || !existsPath(file.file_path)) return [];
  ...
  async saveUploadTemplateWithFile(input: any, file?: Express.Multer.File) {
    ...
    if (previousFilePath && previousFilePath !== filePath) await removePath(previousFilePath, { force: true });
  ...
  async getConfig() {
    ...
    const headersFor = (item: any) => {
      if (!item.file_path || !existsPath(item.file_path)) return [];
    ...
    const templateHealth = allTemplates.map((item: any) => {
      ...
      if (!item.file_path || !existsPath(item.file_path)) issues.push('模板文件不存在，请重新上传');
  ...
  async suggestQuoteMappings(quoteTemplateId: string, importTemplateId: string) {
    ...
    if (!quote || !importTemplate) throw new Error('报价单或导入格式模板不存在');
    const headers = (item: any) => item.file_path && existsPath(item.file_path) ? readRows(item.file_path, item.header_row, item.data_start_row).header.filter(Boolean) : [];
  ...
  async saveMappings(targetTemplateId: string, mappings: any[], uploadTemplateId?: string) {
    ...
    const timestamp = now();
    const shared = (mappings || []).filter((mapping) => SHARED_FORCED_KEYS.has(normalizeForcedKey(mapping.forcedKey)));
    if (uploadTemplateId && shared.length) {
      await this.db.run('DELETE FROM transfer_mappings WHERE target_template_id = ? AND upload_template_id IS NULL AND forced_key IN (?,?,?,?)', [targetTemplateId, '证书单位', '证书地址', '校准日期', '特殊要求']);
      for (const mapping of shared) await this.db.run('INSERT INTO transfer_mappings(id,target_template_id,upload_template_id,source_column,target_column,forced_key,target_cell,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)', [id('mapping'), targetTemplateId, null, null, mapping.targetColumn || null, normalizeForcedKey(mapping.forcedKey), mapping.targetCell || null, timestamp, timestamp]);
    }
    for (const mapping of mappings || []) {
      if (uploadTemplateId && SHARED_FORCED_KEYS.has(normalizeForcedKey(mapping.forcedKey))) continue;
      await this.db.run('INSERT INTO transfer_mappings(id,target_template_id,upload_template_id,source_column,target_column,forced_key,target_cell,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)', [id('mapping'), targetTemplateId, uploadTemplateId || null, mapping.sourceColumn || null, mapping.targetColumn || null, mapping.forcedKey || null, mapping.targetCell || null, timestamp, timestamp]);
    }
  ...
  async deleteTargetTemplate(templateId: string) {
    const template = await this.db.get<any>('SELECT file_path FROM transfer_target_templates WHERE id = ?', [templateId]);
    await this.db.run('DELETE FROM transfer_target_templates WHERE id = ?', [templateId]);
    if (template?.file_path) await removePath(template.file_path, { force: true });
  }
  ...
  private async processQuote(input: any, file: Express.Multer.File, username = '') {
    ...
    const specialRequirements = String(input?.specialRequirements ?? '').trim();
    ...
    if (!quote || !quote.file_path || !existsPath(quote.file_path)) throw new Error('未配置有效的报价单模板');
    if (generationMode !== 'import') {
      if (!orderTemplate || !orderTemplate.file_path || !existsPath(orderTemplate.file_path)) throw new Error('未配置有效的收发委托单模板');
    ...
    if (!importTemplate || !importTemplate.file_path || !existsPath(importTemplate.file_path)) throw new Error('未配置有效的导入格式模板');
    ...
    const sourcePath = path.join(TASK_DIR, id('quote-source'));
    const generatedPath = path.join(TASK_DIR, id('generated-import'));
    let taskId = '';
    ...
    try {
      ...
      const forced = { '证书单位': certificateUnit, '证书地址': certificateAddress, '校准日期': calibrationDate, '特殊要求': specialRequirements };
      ...
      await writeTemplateText(importTemplate.file_path, generatedPath, updates);
      {
        ...
        taskId = id('task'); const folderName = `${namePrefix}_导入格式`; outputFolderPath = taskFolderPath(taskId);
        await fs.promises.mkdir(outputFolderPath, { recursive: true });
        const filename = `${namePrefix}_导入格式.xlsx`; const fileId = id('file'); const outputPath = path.join(outputFolderPath, `${fileId}.xlsx`);
      ...
      await removePath(generatedPath, { force: true });
      if (!completed) {
        if (taskId) {
          const files = await this.db.all<any[]>('SELECT file_path FROM transfer_files WHERE task_id = ?', [taskId]);
          for (const generatedFile of files) if (generatedFile.file_path) await removePath(generatedFile.file_path, { force: true });
        ...
        if (outputFolderPath) await removePath(outputFolderPath, { recursive: true, force: true });
      }
    }
  }
  ...
  private async processTargetOnly(input: any, file: Express.Multer.File, username = '') {
    ...
    if (!sourceTemplate || !sourceTemplate.file_path || !existsPath(sourceTemplate.file_path)) throw new Error('未配置有效的源模板');
    ...
    const decodedName = Buffer.from(file.originalname, 'latin1').toString('utf8');
    if (decodedName && !decodedName.includes('\ufffd')) file.originalname = decodedName;
    const sourcePath = path.join(TASK_DIR, id('source'));
    let taskId = '';
    let folderPath = '';
    try {
      await fs.promises.writeFile(sourcePath, file.buffer);
      ...
      taskId = id('task');
      folderPath = taskFolderPath(taskId);
      ...
      for (const [targetId, group] of groups) {
        ...
        if (!target || !target.file_path || !existsPath(target.file_path)) throw new Error(`转送对象模板“${target?.template_item_name || target?.name || targetId}”文件不存在，请在运行监控中修复`);
        ...
        const dynamicForcedColumns = mappings.filter((item) => ['校准日期', '特殊要求'].includes(normalizeForcedKey(item.forced_key)) && !item.target_cell).map((mapping) => ({ mapping, column: targetBook.header.findIndex((header) => header === String(mapping.target_column || mapping.forced_key || '').trim()) })).filter((item) => item.column >= 0);
        ...
        const outputPath = path.join(folderPath, `${fileId}.xlsx`); await writeTemplateText(target.file_path, outputPath, updates);
      ...
      if (taskId) {
        const files = await this.db.all<any[]>('SELECT file_path FROM transfer_files WHERE task_id = ?', [taskId]);
        for (const generatedFile of files) await removePath(generatedFile.file_path, { force: true });
        await this.db.run('DELETE FROM transfer_files WHERE task_id = ?', [taskId]);
        await this.db.run('DELETE FROM transfer_tasks WHERE id = ?', [taskId]);
      }
      if (folderPath) await removePath(folderPath, { recursive: true, force: true });
      throw error;
    }
  }
  ...
  private async processTwoStage(input: any, file: Express.Multer.File, username = '') {
    ...
    const orderPath = path.join(folderPath, `${orderFileId}.xlsx`);
    ...
    if (generationMode === 'all') {
      ...
      for (const [targetId, group] of groups) {
        ...
        // Quote direct-transfer mappings read columns from the quote workbook,
        // but the target item's four forced fields are source-independent and
        // must always be applied as well.
        const mappings = useOriginalSource
          ? await this.getTargetMappings(targetId, sourceTemplate.id)
          : await this.getTargetMappings(targetId, orderTemplate.id);
        ...
        const dynamicForcedColumnsForTarget = mappings.filter((item) => ['校准日期', '特殊要求'].includes(normalizeForcedKey(item.forced_key)) && !item.target_cell).map((mapping) => ({ mapping, column: targetBook.header.findIndex((header) => header === String(mapping.target_column || mapping.forced_key || '').trim()) })).filter((item) => item.column >= 0);
        ...
        const filename = `${namePrefix}_${targetOutputName(target)}.xlsx`; const fileId = id('file'); const outputPath = path.join(folderPath, `${fileId}.xlsx`);
  ...
  async process(input: any, file: Express.Multer.File, username = '') {
    ...
    folderPath = taskFolderPath(taskId); await fs.promises.mkdir(folderPath, { recursive: true });
    ...
    for (const [targetId, group] of groups) {
      ...
      const dynamicForcedMappings = mappings.filter(
        (item) => ['校准日期', '特殊要求'].includes(normalizeForcedKey(item.forced_key)) && !item.target_cell,
      );
      for (const mapping of mappings.filter((item) => item.forced_key && item.target_cell)) {
        const cell = cellToAddress(mapping.target_cell);
      ...
      const filename = `${namePrefix}_${targetOutputName(target)}.xlsx`;
      const fileId = id('file');
      const outputPath = path.join(folderPath, `${fileId}.xlsx`);
      await writeTemplateText(target.file_path, outputPath, updates);
      const size = (await fs.promises.stat(outputPath)).size;
      ...
      await this.db.run(`UPDATE transfer_tasks SET status='completed', skip_detail_json=?, completed_at=? WHERE id=?`, [JSON.stringify(serializeSkipDetails(skipDetails)), now(), taskId]); await removePath(sourcePath, { force: true }); return { task: { id: taskId, folderName, totalRows: source.data.length, matchedRows: source.data.length - skipped, skippedRows: skipped, status: 'completed', skipDetailJson: serializeSkipDetails(skipDetails) }, files: generated };
    } catch (error) {
      await removePath(sourcePath, { force: true });
      if (taskId) {
        const files = await this.db.all<any[]>('SELECT file_path FROM transfer_files WHERE task_id = ?', [taskId]);
  ...
  async deleteTask(taskId: string, username = '') {
    ...
    const files = await this.db.all<any[]>('SELECT file_path FROM transfer_files WHERE task_id = ?', [taskId]);
    const taskDirs = new Set<string>([taskFolderPath(taskId)]);
    // Delete by the paths stored in the database first. This covers legacy
    // tasks whose folder name can exceed MAX_PATH or no longer match metadata.
    for (const file of files) {
      if (!file.file_path) continue;
      taskDirs.add(path.dirname(file.file_path));
      await removePath(file.file_path, { force: true });
    }
    // Keep compatibility with pre-fix tasks that used `${id}-${folderName}`.
    taskDirs.add(path.join(TASK_DIR, `${taskId}-${task.folder_name || ''}`));
    for (const taskDir of taskDirs) await removePath(taskDir, { recursive: true, force: true });
    await this.db.run('DELETE FROM transfer_files WHERE task_id = ?', [taskId]);
    await this.db.run('DELETE FROM transfer_tasks WHERE id = ?', [taskId]);
  }
  ...
  async getZip(taskId: string, fileIds?: string[]) {
    ...
    const entries = [];
    for (const item of selected) if (existsPath(item.file_path)) entries.push({ name: path.basename(item.filename), data: await fs.promises.readFile(toFilesystemPath(item.file_path)) });
